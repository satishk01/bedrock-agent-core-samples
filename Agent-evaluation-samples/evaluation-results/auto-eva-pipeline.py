...
region = "us-east-1"
agent_id = "ac_eval_strands2-x2psSfGYGb"  # UPDATE: Your agent ID
agent_arn = "arn:aws:bedrock-agentcore:us-east-1:319645399632:runtime/ac_eval_strands2-x2psSfGYGb"  # UPDATE: Your agent ARN
...
print(f"✓ Completed {len(invocation_results)} agent invocations\n")
...
print("✓ Wait complete. Starting evaluations...\n")
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if "No spans found" in str(e) and attempt < max_retries:
                print(f"⚠ No spans found. Retrying in {retry_delay}s...")
...
for i, result in enumerate(batch_results, 1):
    ...
    if 'error' in result:
        ...
        for ev in result['evaluations']:
            print(f"      • {ev['evaluator']}: {ev['value']} ({ev['label']})")
    else:
        print(f"    Status: ⚠ No evaluation results")
...
# ============================================================================
# STEP 5: Save results to JSON for further analysis
# ============================================================================
from datetime import datetime
import json

timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
results_file = f"session-results_{timestamp}.json"
...
# Also save to eval_data folder for eval-auto.py analysis
try:
    import os
    os.makedirs('eval_data', exist_ok=True)
    
    eval_data_file = f"eval_data/pipeline_eval_{timestamp}.json"
    
    # Convert to eval-auto.py compatible format
    eval_auto_data = []
    for result in batch_results:
        if 'evaluations' in result and result['evaluations']:
            for ev in result['evaluations']:
                eval_auto_data.append({
                    'session_id': result['session_id'],
                    'score': ev.get('value'),
                    'explanation': ev.get('explanation', ''),
                    'metadata': {
                        **result.get('metadata', {}),
                        'evaluator_name': ev.get('evaluator'),
                        'label': ev.get('label', ''),
    ...
    with open(eval_data_file, 'w', encoding='utf-8') as f:
        json.dump(eval_auto_data, f, indent=2, default=str)
    
    print(f"💾 Eval data saved to: {eval_data_file}")
    print(f"   (Ready for eval-auto.py analysis)")
except Exception as e:
...
try:
    ...
    print("\n⚠ dashboard_generator.py not found - skipping dashboard generation")
except Exception as e:
...
print(f"{'='*80}")
print(f"1. {results_file} - Complete session results (JSON)")
print(f"2. {eval_data_file} - Formatted for eval-auto.py analysis")
if 'dashboard_file' in locals():
    print(f"3. {dashboard_file} - Interactive HTML dashboard")
print(f"\nNext steps:")
print(f"  • Open dashboard in browser for visualizations")
print(f"  • Run 'python3 eval-auto.py' to analyze low-scoring evaluations")
print(f"  • Review {results_file} for detailed results")
print(f"{'='*80}\n")
